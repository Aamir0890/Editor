...
import { faPen, faArrowCircleLeft, faArrowCircleRight, faCopy, faLightbulb,faImage,faSmile, faSave } from '@fortawesome/free-solid-svg-icons';
...
const TextEditorComponent: React.FC = () => {
  ...
  const linkToCopy = 'https://www.groupgreeting.com/sign/d12e60c5ec0db9c';
  ...
  const showNextCard = () => {
    ...
    setIsEditing(false);
    setCurrentEditingId(null); // Reset editing state when switching cards
  };

  const handleAddTextItem = () => {
    if (isEditing && currentEditingId !== null) {
      // If editing, save the text and close the editor
      setIsEditing(false);
  ...
  return (
    <div className="card-slider">
      ...
      >
        ...
        <FontAwesomeIcon icon={isEditing && currentEditingId !== null ? faSave : faPen} className="icon" fontSize={20} />
        <span className="text">{isEditing && currentEditingId !== null ? 'Save Text' : 'Add Text'}</span>
        </button>
            ...
            >
              <div
                style={{
                  ...
                  zIndex: 1,
                  border: currentEditingId === item.id ? '1px dashed #ccc' : 'none',
                  padding: '5px',
                  minHeight: '20px',
